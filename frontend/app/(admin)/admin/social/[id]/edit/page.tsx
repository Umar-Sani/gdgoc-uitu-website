...
export default function EditSocialPostPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/social/posts/${params.id}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(r => r.json())
      .then(res => {
        if (res.data) {
          setPlatform(res.data.platform ?? 'instagram');
          setCaption(res.data.caption ?? '');
          setHashtags(res.data.hashtags?.map((h: string) => `#${h}`).join(' ') ?? '');
          setTags(res.data.tags?.join(', ') ?? '');
          setScheduledAt(res.data.scheduled_at ? new Date(res.data.scheduled_at).toISOString().slice(0, 16) : '');
  ...
  async function handleSave(newStatus: string) {
    ...
    try {
      const res = await fetch(`${API_URL}/api/social/posts/${params.id}`, {
        method: 'PATCH',
        headers: {
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5 capitalize">Status: {status}</p>
          </div>
        </div>
